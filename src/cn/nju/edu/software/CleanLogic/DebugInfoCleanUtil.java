...
public class DebugInfoCleanUtil {
    ...
    public void cleanDebufInfo(int sid,int eid,String projectName){
    ...
    private List<String> projetcName(){
        ...
        ResultSet set=null;
        try {
            PreparedStatement prepar=connection.prepareStatement("select DISTINCT debug_targetfrom debug_info");
            //把sql语句发送到数据库，得到预编译类的对象，这句话是选择该student表里的所有数据
            set=prepar.executeQuery();
            while(set.next()) {
